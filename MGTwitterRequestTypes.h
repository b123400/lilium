...
typedef enum _MGTwitterRequestType {
  ...
  MGTwitterHomeTimelineRequest, // latest statuses from the home timeline
	...
	MGNewTwitterUserListsRequest,
	MGTwitterUserListCreate,
	MGTwitterListStatusRequest,  //requesting status of list
	MGNewTwitterListMemberRequest , //member of a list
	MGTwitterReportSpamRequest,     //report spam
	MGTwitterGetBlockingRelationshipRequest,//get blocking relationship
...
typedef enum _MGTwitterResponseType {
	...
	MGNewTwitterUserLists       = 13,   //Because the original one sucks
	MGTwitterListsList			= 14,
	MGTwitterTrendsResult       = 15,   //Trend is not same the as search result
	MGTwitterSavedSearchesResult= 16,   //Multi searches
	MGTwitterSavedSearch        = 17,   //A single search
