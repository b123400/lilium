...
-(void)getAccessTokenWithOauthToken:(NSString*)token verifier:(NSString*)verifier{
	[super getAccessTokenWithOauthToken:token verifier:verifier withURL:[NSURL URLWithString:@"https://api.twitter.com/oauth/access_token"]];
}

- (void)requestTokenTicket:(OAServiceTicket *)ticket didFinishWithData:(NSData *)data {
	[super requestTokenTicket:ticket didFinishWithData:data];
	NSURL *authorizeURL = [NSURL URLWithString:[NSString stringWithFormat:@"https://api.twitter.com/oauth/authorize?oauth_token=%@&force_login=true",[requestToken key]]];
	if(self.delegate){
